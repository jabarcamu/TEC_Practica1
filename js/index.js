...
const pack = d3.pack()
    .size([960,700])
    .padding(5); //distance between the circles

const bubbleData = pack(rootNode).descendants();

// create ordinal scale
const color1 = d3.scaleOrdinal(["#d1c4e9", "#b39ddb", "#9575cd"])


// join data and add group for eahc node
...
nodes.append("circle")
  ...
  .attr("stroke", "white")
  .attr("stroke-width", 2)
  .attr("fill", d => color1(d.depth))

nodes.filter(d => !d.children)
    .append("text")
    .attr("text-anchor", "middle")
    .attr("dy", "0.3em") // offset in the y direction
    .attr("fill", "white")
